    ...
    function process() {
        ...
        switch ($this->action) {
            ...
            case ItemServiceActions::Archive:
                ...
                break;
            case ItemServiceActions::UploadMedia:
                $resp = $this->UploadMedia();
                break;
            case ItemServiceActions::Media:
                $resp = $this->GetMedia();
                break;
        ...
        if ($this->applicationId != "") {
            switch ($this->paramArr[0]) {
                ...
                case "Archive":
                    ...
                    break;
                case "UploadMedia":
                    $this->action = ItemServiceActions::UploadMedia;
                    break;
                case "Media":
                    $this->action = ItemServiceActions::Media;
                    break;
...
//        $file = "cache/" . $this->applicationId . ".js";
    ...
    function getItemsByType() {
        ...
        $key = $this->paramArr[2];
        $query = "SELECT id, json FROM items WHERE active = 1 AND template_id = $type_id AND application = '$this->applicationId' ORDER BY sort_order";
        $sql = new sqlQuery($this->conn, $query);
        //return json_encode($sql, JSON_PRETTY_PRINT);
        $keys = array();
        $values = array();
...
//        array_push($items, $this);
    ...
    function updateItemsMissingFields(){
        $output = array();
        $query = "SELECT * FROM items WHERE application = '$this->applicationId'";
        $sql = new sqlQuery($this->conn, $query);
        foreach($sql->rows as $item){
            $template_id = $item["template_id"];
            $item_id = $item["id"];
            $item_data_query = "select item_field_id from item_data WHERE item_id = ".$item_id;
            $item_template_fields_query = "select id from item_template_fields WHERE template_id = ".$template_id;
            $sql_data = new sqlQuery($this->conn, $item_data_query);
            $sql_fields = new sqlQuery($this->conn, $item_template_fields_query);
            $data_ids = __::pluck($sql_data->rows, "item_field_id");
            $fields_ids = __::pluck($sql_fields->rows, "id");
            $missing_ids = __::difference($fields_ids, $data_ids);
            //array_push($output, $sql_data);
            //array_push($output, $sql_fields);
            //array_push($output, $data_ids);
            //array_push($output, $fields_ids);
            if(sizeof($missing_ids > 0)){
            array_push($output, $item_id);
            array_push($output, $missing_ids);
                foreach($missing_ids as $id){
                $insert_query = "INSERT INTO item_data (item_id, item_field_id, value, active) VALUES ($item_id, $id, '', 1)";
                $insert_sql = new sqlQuery($this->conn, $insert_query, sqlQueryTypes::sqlQueryTypeINSERT);
                array_push($output, $insert_sql);
                }
            }
        }
        return json_encode($output, JSON_PRETTY_PRINT | JSON_HEX_APOS);
...
//
//    function updateJson() {
//        $sql = $this->query('SELECT id FROM items active = 1 LIMIT 5');
//        foreach ($sql->rows as $row) {
//            $id = $row["id"];
//            print_r($id . ",");
//            $this->saveJson($id);
